...
| exp_055 | configs/exp_055.yaml | — | — | — | — | — | — | — | — | BINNED REDSHIFT HEAD (prediction_type: binned, 24 log(1+z) bins over z≤4.0, λ_refine 0.3, λ_nmad 0.7, label_smoothing 0.05) on the known-good v2_Q1 pipeline (exp_032 config, frozen DESI AE, 12×1024 head, lr 1e-4, bs 128, 400 ep, pat 50, wd 5e-5, zscore norm). First controlled test of the binned head. Purpose: kill catastrophic outliers (exp_032 η=15.17%, target <1%) while holding NMAD ≤0.016. z_bin_max=4.0 covers the full v2_Q1 range (max z=3.9998). |
