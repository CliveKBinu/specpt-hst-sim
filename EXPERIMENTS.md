...
| exp_054 | configs/exp_054.yaml | 6hkuxeh0 | rosy-breeze-111 | 0.43671 | 0.39394 | 67.45% | 0.0103 | 0.53843 | 0.90248 | FROZEN SIMV4A-ADAPTED AE + exp_032 head on simv4a. CATASTROPHIC — test NMAD 0.39394, η 67.45%, RMSE 0.53843, bias 0.01030. Best val NMAD 0.43671 @ ep71, early stop. Third consecutive simv4a failure (052/053/054): AE adaptation does NOT recover redshift signal. VERDICT: stop simv4a redshift-head work; investigate v4a pipeline (label alignment / spectrum construction / normalization). |
| exp_056 | configs/exp_056.yaml | qsk6khh9 | vague-butterfly-113 | 0.42347 | 0.39869 | 67.96% | 0.0007 | 0.53329 | — | EXACT exp_032 config on simv4a (DESI combined AE, zscore norm, random split, 12×1024 head, lr 1e-4) — the definitive control. CATASTROPHIC: test NMAD 0.39869, η 67.96%, RMSE 0.53329, bias 0.00073. Best val NMAD 0.42347 @ ep74. FOURTH consecutive simv4a failure (052 scratch / 053 regridded AE / 054 simv4a-adapted AE / 056 exact best config) — all collapse to ~0.39. VERDICT: the simv4a DATA FILE itself is definitively the cause (label alignment / spectrum construction / normalization), NOT the model, AE, or split. No further simv4a model work until the v4a pipeline is fixed. Tigris job 83811. |
| exp_055 | configs/exp_055.yaml | j94jfi0y | scarlet-snowflake-112 | 0.02756 | 0.03288 | 14.90% | 0.0051 | 0.33133 | — | BINNED REDSHIFT HEAD on v2_Q1 (24 uniform log(1+z) bins over z≤4, λ_refine 0.3, λ_nmad 0.7, label_smoothing 0.05, exp_032 pipeline). GATE FAILED: test NMAD 0.03288 (4.2× worse than exp_032 0.00785; needed ≤0.016), test η 14.90% (vs exp_032 15.17% — only 0.3pp better; needed <1%), RMSE 0.33133, bias 0.0051. Early stop ep82 (runtime ~18 min on tigris GH200). Binned head does NOT beat the pretrained continuous regression head — matches the reference implementation's empirical finding. Marginal outlier benefit (15.17→14.90) does not justify 4× NMAD degradation. Tigris job 83810. |
...
| tracka_control_z_transfer | configs/tracka_control_z.yaml | 41788 | — | — | — | — | — | — | — | TRANSFER: head-only fine-tuning of sim-trained control on real 3D-HST. AE frozen, head trainable, lr=1e-5, 150 ep, patience 25. Early stop ep45. Test NMAD 0.26141, η 57.14%, RMSE 0.6192, bias -0.139. Pred std 0.240 vs true 0.554 (std_ratio 0.433). |
| tracka_small_z_transfer | configs/tracka_small_z.yaml | 41789 | — | — | — | — | — | — | — | TRANSFER: head-only fine-tuning of sim-trained small on real 3D-HST. AE frozen, head trainable, lr=1e-5, 150 ep, patience 25. Early stop ep50. Test NMAD 0.26422, η 57.15%, RMSE 0.6108, bias -0.109. Pred std 0.234 vs true 0.554 (std_ratio 0.422). |
| tracka_tiny_z_transfer | configs/tracka_tiny_z.yaml | 41790 | — | — | — | — | — | — | — | TRANSFER: head-only fine-tuning of sim-trained tiny on real 3D-HST. AE frozen, head trainable, lr=1e-5, 150 ep, patience 25. Early stop ep35. Test NMAD 0.25668, η 55.96%, RMSE 0.6018, bias -0.113. Pred std 0.207 vs true 0.554 (std_ratio 0.374). Best head-only but strong shrinkage. |
...
| exp_056 | configs/exp_056.yaml | — | — | — | — | — | — | — | — | EXACT exp_032 config on simv4a — the missing control. Byte-identical to the 0.00785 best (DESI combined AE, 12×1024 head, lr 1e-4, bs 128, 400 ep, pat 50, wd 5e-5, ZSCORE normalization, RANDOM split, no freeze_backbone key) with ONLY data.path → grism_training_simv4a.parquet. Unlike exp_053 (regridded AE + grouped split) and exp_054 (simv4a-adapted AE + grouped), this isolates the data file itself: if it collapses to ~0.39, simv4a data is definitively the cause, not AE/split. (Note: train.py never reads the normalization key — all runs z-score at load.) |
| exp_057 | configs/exp_057.yaml | — | — | — | — | — | — | — | — | RERUN of the exp_032 config on v2_Q1 — byte-identical to the 0.00785 best (DESI combined AE, 12×1024 head, lr 1e-4, bs 128, 400 ep, pat 50, wd 5e-5, zscore norm, random split). Reproducibility / seed-variance check (exp_013_rerun showed seed variance 0.01382→0.02024; want to know the variance band around 0.00785). Note: runs on tigris GH200 vs exp_032's original cluster — any hardware delta noted. |
| exp_058 | configs/exp_058.yaml | — | — | — | — | — | — | — | — | EXP_032 CONFIG ON SIMV4A_V2 — fixed line detectability (integrated_line_snr selection, 54.7% ≥ SNR5 vs old 2.1%). Byte-identical exp_032 pipeline (DESI combined AE, 12×1024 head, lr 1e-4, zscore norm, random split) with only data.path → grism_training_simv4a_v2.parquet. Definitive test: does fixing line detectability recover redshift learning on simv4a? |
| exp_059 | configs/exp_059.yaml | — | — | — | — | — | — | — | — | REGULARIZATION A/B #1 — head dropout_rate 0.1→0.3 (via new model.dropout_rate key, frozen AE dropout stays 0.1). Same simv4a_v2 data + exp_032 pipeline. Tests: does more dropout curb the overfitting (train loss→0.18 / val loss→0.54) seen in exp_058? |
| exp_060 | configs/exp_060.yaml | — | — | — | — | — | — | — | — | REGULARIZATION A/B #2 — num_mlp_blocks 12→5 (less head capacity). Same simv4a_v2 data + exp_032 pipeline. Tests: does reduced capacity stop memorization of the 45% z_quality=-1 spectra? |
| exp_061 | configs/exp_061.yaml | — | — | — | — | — | — | — | — | REGULARIZATION A/B #3 — weight_decay 5e-5→1e-4. Same simv4a_v2 data + exp_032 pipeline. Tests: does stronger L2 penalty improve generalization vs exp_058 baseline (NMAD 0.09250)? |
