...
3. Compare to best NMAD from SOUL.md
...
Workflow:
1. Read ./EXPERIMENTS.md and SOUL.md for full history
...
2. SSH + submit (Duo MFA required). Use a 30-min timeout:
...
Retry logic:
- Duo timeout: retry up to 3x with 60s delays
- SSH connection failure: retry up to 3x with 30s delays
- sbatch failure: report error, do not retry
...
job_in_queue = str(job_id) in result.stdout
...
and `runner_result` (job_id + status).
...
- Read SOUL.md, ./EXPERIMENTS.md, ./jobs.csv, ./README.md
...
```
Count completed/running by counting ROWS in the respective EXPERIMENTS.md tables.
...
with open(os.path.join(repo, "SOUL.md")) as f:
    soul_after = f.read()
...
If still failing, write a `.memory_verification_failed` marker file.

### Error Handling
- If analyst fails: log error, try to continue with experimenter using last known state
- If experimenter fails: retry once, then stop
...
- If verification step fails: retry the corresponding subagent step once; on second failure, write a marker file and abort the cycle
